...
impl<'a> IntkeyState<'a> {
    ...
    pub fn get(&mut self, name: &str) -> Result<Option<u32>, ApplyError> {
        ...
        match d {
            Some(packed) => {
                //let packed = String::from_utf8(data).unwrap().from_hex().unwrap();
    ...
    pub fn set(&mut self, name: &str, value: u32) -> Result<(), ApplyError> {
        ...
        //let s: Vec<String> = packed.iter().map(|b| format!("{:02X}", b)).collect();

        //self.context.set_state(&IntkeyState::calculate_address(name), s.join("").as_bytes())?;
