            ...
            Some(packed) => {
                ...
                let status = match map.get(&Key::Text(Text::Text(String::from(name)))) {
                    Some(v) => match v {
                        ...
                        _ => Err(ApplyError::InternalError(String::from("Value returned from state is wrong type.")))
                    },
                    None => Ok(None)
                };
        ...
        match payload.get_verb() {
            Verb::Set => {
                match state.get(payload.get_name()) {
                    Ok(Some(_v)) => return Err(ApplyError::InvalidTransaction(format!("{} already set", payload.get_name()))),
                    Ok(None) => (),
