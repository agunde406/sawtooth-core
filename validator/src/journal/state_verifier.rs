...
/// Validate that the existing state is valid, if not, rebuild it from the committed blocks
pub fn verify_state(
    ...
    let previous_state_root = prev_state_root.unwrap_or_else(|| initial_state_root.to_string());
...
fn search_for_present_state_root(
    ...
    state_view_factory: &StateViewFactory,
    initial_state_root: &str,
) -> Result<(Option<BlockPair>, Option<String>), StateVerificationError> {
    // If there is no chain to process, then we are done.
    let block = match commit_store.get_chain_head() {
        Ok(block) => block,
        Err(DatabaseError::NotFoundError(_)) => return Ok((None, None)),
        Err(err) => {
    ...
    for block in commit_store.get_block_by_height_iter(None, ByHeightDirection::Increasing) {
        if state_view_factory
            ...
            .is_err()
        {
            return Ok((Some(block), Some(previous_state_root)));
        }
    ...
    Err(StateVerificationError::InvalidChainError(
        "Chain head state both missing but all blocks had state root present".to_string(),
    ))
}
...
fn process_blocks(
    ...
    let mut previous_state_root = initial_state_root.to_string();
    warn!("Start block processes at {}", previous_state_root);
    for block in blocks {
...
fn execute_batches(
    ...
    warn!("Waiting for execution results");
    let mut execution_results = vec![];
    loop {
        warn!("wait for results");
    ...
    warn!("Deal with all execution results");
    let mut changes = vec![];
